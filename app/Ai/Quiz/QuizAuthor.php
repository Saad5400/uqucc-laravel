...
class QuizAuthor
{
    ...
    private const INSTRUCTIONS = <<<'PROMPT'
        ...
        القواعد:
        - اكتب بالعربية الفصحى المبسطة، وأبقِ المصطلحات التقنية الإنجليزية كما تُكتب عادة.
        - سؤال واحد واضح له إجابة صحيحة واحدة لا لبس فيها، وثلاثة بدائل خاطئة معقولة (ليست هزلية ولا واضحة الخطأ).
        - مستوى الصعوبة متوسط: يجيب عنه الطالب المتابع دون أن يكون بديهياً — ومال إلى الأساسيات المشتركة ما لم يكن الموضوع تخصصياً.
        - لا تعتمد على معلومات قد تتغير مع الزمن (إصدارات حديثة، أسعار، أشخاص).
        - الحدود الصارمة: السؤال 300 حرف كحد أقصى، كل خيار 100 حرف كحد أقصى، الشرح 200 حرف كحد أقصى.
    ...
    private function buildPrompt(QuizTopic $topic): string
    {
        ...
        if ($topic->is_spotlight) {
            $prompt .= "\n".'هذا موضوع تخصصي (يوم التخصص الأسبوعي) — يجوز سؤال أعمق من المعتاد في هذا التخصص.';
        }
