 ...
 *
 * The joint announcement is why groups attach to intakes through a pivot: its
 * twelve programme groups serve batches ٤٦ and ٤٧ alike, so they are created
 * once and attached twice rather than duplicated and kept in sync by hand.
 *
 ...
 * other content. Re-running is safe — the whole import is skipped once any
 * intake exists, so a later `migrate` never resurrects a supervisor an admin
 * removed.
 *
...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        if (app()->runningUnitTests() || Cohort::query()->exists()) {
            return;
        }

        $cohorts = [];

        foreach ($this->cohorts() as $key => $definition) {
            $cohorts[$key] = Cohort::create([
        ...
        foreach ($this->groups() as $definition) {
            ...
            $group->cohorts()->attach(array_map(fn (string $key) => $cohorts[$key]->id, $definition['cohorts']));

            foreach ($definition['supervisors'] as [$section, $name, $contacts]) {
                $group->supervisors()->create([
                    ...$this->contactColumns($contacts),
                    'name' => $name,
                    'section' => $section,
                    'is_available' => ! in_array('unavailable', $contacts, true),
                ]);
            }
        }
    }
    ...
    /**
     * The intakes, newest first — a batch is what a student picks, and what
     * decides which global group they belong in.
     *
    ...
    private function cohorts(): array
    {
        $jointRequirements = [
            ...
            'اسم التخصص',
            'رقم الدفعة',
            'أول ٣ أرقام من الرقم الجامعي ظاهرة في الصورة',
        ];
        $jointNote = 'التزم بوسيلة التواصل المحددة لكل مشرف. وإن لم يصلك رد لفترة طويلة، تواصل مع مشرف آخر.';

        return [
            'c48' => [
                'name' => 'دفعة ٤٨',
                'featured' => true,
                'description' => 'مبارك لكم قبولكم في كلية الحاسبات بجامعة أم القرى. انضم إلى القروب العام لدفعتك وإلى قروب تخصصك.',
            ...
            'c47' => [
                'name' => 'دفعة ٤٧',
                'featured' => false,
                'description' => 'قروبات دفعة ٤٧ في كلية الحاسبات: القروب العام للدفعة، وقروب تخصصك في فرعك.',
                'note' => $jointNote,
                'requirements' => $jointRequirements,
            ],
            'c46' => [
                'name' => 'دفعة ٤٦',
                'featured' => false,
                'description' => 'قروبات تخصصات دفعة ٤٦ في كلية الحاسبات. ادخل على قروب تخصصك الخاص بشطرك وفرعك.',
                'note' => $jointNote,
                'requirements' => $jointRequirements,
            ],
        ];
    ...
    /**
     * Every group, with the intakes it serves.
     *
     * @return array<int, array<string, mixed>>
     */
    private function groups(): array
    {
        return [...$this->cohort48Groups(), ...$this->cohort47Groups(), ...$this->jointGroups()];
    }

    /**
     * @return array<int, array<string, mixed>>
     */
    private function cohort48Groups(): array
    {
    ...
    /**
     * The list the standalone دفعة ٤٧ page carried — that batch's global group.
     * The names it kept commented out come across as unavailable rather than
     * missing; that flag is exactly what the comments were standing in for.
     *
     * @return array<int, array<string, mixed>>
     */
    private function cohort47Groups(): array
    {
        $men = SupervisorSection::Men;
        $women = SupervisorSection::Women;
    ...
    /**
     * The programme groups the college publishes for دفعة ٤٦ و٤٧ together:
     * created once, attached to both intakes.
     *
     * @return array<int, array<string, mixed>>
     */
    private function jointGroups(): array
    {
        $men = SupervisorSection::Men;
        $women = SupervisorSection::Women;
